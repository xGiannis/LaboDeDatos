...
carpeta = "~/Escritorio/LaboDeDatos/Clase 12/Visualización y Análisis Exploratorio de Datos (Parte 1)/"
...
fig, ax = plt.subplots() #-----> aca si le paso cosas al subplot() puedo tener mas de un graf.
...
wine = pd.read_csv(carpeta+"wine.csv", sep = ";") # con sep indicamos que el 
                                                    # separador es ;
plt.scatter(data = wine, x='sulphates', y='pH')
###
fig, ax = plt.subplots() #-----> aca si le paso cosas al subplot() puedo tener mas de un graf.
...
ax.set_title('sulfato vs pH') # Titulo del gráfico
ax.set_xlabel('sulfato ', fontsize='medium')   # Nombre eje X           
ax.set_ylabel('pH', 
              fontsize='medium')           

# ¿Existe alguna relación entre el pH de los vinos (pH) y alguna de las otras variables? 
#PARECIERA Q SI EN SULFATO, PERO NO CREO! EL CASO EXTRA TIENE MUCHISIMO MAS CLORIDIO Q LOS DEMAS
tamanoBurbuja = 5 

# Muestrelo gráficamente 
# Justifique la elección del tipo de gráfico.

plt.scatter(data=wine, x='sulphates', y='pH', s='chlorides')
...
tamanoBurbuja = 520 # Cuanto queremos modificar
                     # el tamaño de cada burbuja

ax.scatter(data=wine, x='sulphates', 
           y='pH', s=wine['chlorides']*tamanoBurbuja)

ax.set_title('Relación entre tres variables')
ax.set_xlabel('sulfato', fontsize='medium')                       
ax.set_ylabel('pH', 
              fontsize='medium')    
...
precioBio="precioBiodiesel.csv"

df = pd.read_csv(carpeta+precioBio) # Cargamos los datos
...
ax.plot('Periodo','Precio', data=df, marker="o",color="red")
plt.grid()

plt.xticks(rotation=90)
ax.set_title('Precio de Biodiesel a lo largo del tiempo')
ax.set_xlabel('Año', fontsize='medium')                       
...
# Agrega titulo, etiquetas a los ejes y limita el rango de valores de los ejes    
ax.set_title('Tips en bares dependiendo de sexo y dia')
ax.set_xlabel('Monto tip $')
